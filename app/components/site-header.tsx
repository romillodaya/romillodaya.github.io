...
export function SiteHeader() {
 ...
 return <header className="site-header"><a className="wordmark" href={assetPath('/')} aria-label="Romil Lodaya, home">romil<span className="brand-star" aria-hidden="true">✳</span></a><nav aria-label="Primary navigation">{navigation.map(item => <a key={item.href} href={assetPath(item.href)} aria-current={pathname === item.href || pathname.startsWith(`${item.href}/`) ? 'page' : undefined}>{item.label}</a>)}</nav><ThemeSwitch /></header>;
}
